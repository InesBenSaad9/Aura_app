...
public class AdaptiveWorkspaceManager {
    ...
    // ── Theme enum — utilisé dans AIAnalysisController ────────
    ...
    public static class Palette {
        ...
        public Palette(String background, String sidebar, String accent, String accentSoft,
                       ...
                       String cardBackground, String borderColor, String glowColor,
                       String nomTheme) {
    ...
    // ── INITIALE — palette AURA de base (avant analyse) ───────
    public static final Palette PALETTE_INITIALE = new Palette(
            "#0A0C0F", "#111418", "#1BBFA8",
            "rgba(27,191,168,0.10)", "#6B5FD4",
            "#F4F7F8", "#A8B4C0", "#111418",
            "rgba(27,191,168,0.15)", "rgba(27,191,168,0.18)",
            "AURA Initial"
    );

    // ── TOXIC PULSE — Énergisé ────────────────────────────────
    public static final Palette PALETTE_ENERGISE = new Palette(
            "#080E08", "#0C160C", "#8eff01",
            "rgba(142,255,1,0.12)", "#8b53fe",
            "#E8FFD0", "#7ACC00", "#0F1A0F",
            "rgba(142,255,1,0.3)", "rgba(142,255,1,0.35)",
            "Toxic Pulse ⚡"
    );

    // ── SOFT GLOW — Fatigué ───────────────────────────────────
    public static final Palette PALETTE_FATIGUE = new Palette(
            "#130E18", "#1A1222", "#C8A0D4",
            "rgba(200,160,212,0.12)", "#F4B8C8",
            "#F0E4F4", "#9E86AA", "#1C1428",
            "rgba(200,160,212,0.25)", "rgba(200,160,212,0.3)",
            "Soft Glow 🌸"
    );

    // ── BREATHE — Calme ───────────────────────────────────────
    public static final Palette PALETTE_CALME = new Palette(
            "#0E1512", "#121C18", "#6B9E8A",
            "rgba(107,158,138,0.12)", "#E8A882",
            "#EEF4F0", "#7AA898", "#141E1A",
            "rgba(107,158,138,0.25)", "rgba(107,158,138,0.3)",
            "Breathe 🍃"
    );

    // ── BLOOM — Stressé ───────────────────────────────────────
    public static final Palette PALETTE_STRESSE = new Palette(
            "#100D18", "#160F22", "#C8B4E8",
            "rgba(200,180,232,0.12)", "#F4B8C8",
            "#EDE8F4", "#A090C0", "#1A1230",
            "rgba(200,180,232,0.25)", "rgba(200,180,232,0.3)",
            "Bloom 🌿"
    );
    ...
    public void register(HBox rootHBox, VBox sidebar, List<Button> navButtons) {
        ...
        System.out.println("AdaptiveWorkspace : enregistré ✓");
    }
    ...
    // ─────────────────────────────────────────────────────────
    // POINT D'ENTRÉE — appelé depuis AIAnalysisController
    // ─────────────────────────────────────────────────────────
    public void appliquerDepuisHumeur(String moodLabel) {
        ...
        System.out.println("AdaptiveWorkspace → " + paletteActuelle.nomTheme);
        Platform.runLater(this::appliquerPaletteUI);
        jouerSon(humeur);
        notifierListeners();
    }

    // Compatibilité avec EmotionalScoreEngine.ThemeUI
    ...
    private void appliquerPaletteUI() {
        if (rootHBox == null) {
            System.out.println("⚠ register() pas encore appelé");
            return;
        }
        ...
        if (sidebar != null) {
            sidebar.setStyle(
                    "-fx-background-color: " + paletteActuelle.sidebar + "; " +
                            "-fx-border-width: 0;"
            );
        }
        ...
        System.out.println("✓ Thème appliqué : " + paletteActuelle.nomTheme);
    }
    ...
    private void jouerSon(String humeur) {
        try {
            ...
            if (url != null) {
                ...
                System.out.println("Son non trouvé : " + soundFile + " (ajoute les .mp3 dans resources/sounds/)");
            }
        } catch (Exception e) {
    ...
    // ─────────────────────────────────────────────────────────
    // GETTERS — tous nécessaires dans AIAnalysisController
    // ─────────────────────────────────────────────────────────
    private void notifierListeners() { for (Runnable r : listeners) r.run(); }

    public Palette getPaletteActuelle()    { return paletteActuelle; }
    public Theme   getThemeActuel()        { return themeActuel; }          // ← utilisé dans AIAnalysisController
    ...
    public String  getTextPrimaryColor()   { return paletteActuelle.textPrimary; }   // ← manquait avant
