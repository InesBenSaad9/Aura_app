 ...
 * Remplace OpenAI — gratuit, sans quota strict
 *
 * Clé API gratuite : https://aistudio.google.com/app/apikey
 */
public class GeminiService {

    // ⚠️ Remplace par ta clé Gemini (gratuite sur aistudio.google.com)
    ...
    /**
     * Génère une réponse AURA selon la transcription + le mood
     *
     * @param transcription  ce que l'utilisateur a dit (depuis Whisper)
     * @param moodLabel      mood détecté : "stressé", "fatigué", "calme", "énergisé"
     * @return réponse textuelle courte d'AURA (1-2 phrases)
     */
    public String genererReponse(String transcription, String moodLabel) {
        try {
            ...
            System.out.println("[Gemini] Envoi requête...");
    ...
    private String buildPrompt(String transcription, String moodLabel) {
        String contexte = switch (moodLabel == null ? "" : moodLabel.toLowerCase().trim()) {
            case "stressé", "stresse"   ->
                    "L'utilisateur est stressé. Réponds de façon très douce et rassurante. Propose de souffler.";
            case "fatigué", "fatigue"   ->
                    "L'utilisateur est fatigué. Sois calme et encourageante. Suggère une pause.";
            case "calme"                ->
                    "L'utilisateur est calme. Sois positive et aide-le à rester focus.";
            case "énergisé", "energise" ->
                    "L'utilisateur est plein d'énergie. Sois dynamique et enthousiaste !";
            case "focalisé", "focalise" ->
                    "L'utilisateur est concentré. Sois précise et efficace.";
            default ->
                    "Sois bienveillante et aide l'utilisateur.";
        };

        String texteUser = (transcription != null && !transcription.isBlank())
                ? transcription
                : "Bonjour AURA";
    ...
    private String httpPost(String jsonBody) {
        try {
            ...
            System.out.println("[Gemini] HTTP " + code);

            if (code == 400) {
                System.err.println("[Gemini] ❌ Clé API invalide !");
                return null;
            }
            if (code != 200) {
                try (InputStream err = conn.getErrorStream()) {
                    if (err != null)
                        System.err.println("[Gemini] Erreur : " + new String(err.readAllBytes()));
                }
                return null;
            }
    ...
    // ─── Parse JSON réponse Gemini ────────────────────────────────────────────

    /**
     * Extrait le texte depuis la réponse JSON Gemini
     * Format : {"candidates":[{"content":{"parts":[{"text":"réponse ici"}]}}]}
     */
