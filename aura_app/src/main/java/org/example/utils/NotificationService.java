...
public class NotificationService {
    ...
    public static void prescriptionReceived(String med) {
        getInstance().show(Type.PRESCRIPTION, "Nouveau médicament", med);
    }
    public static void appointmentReminder(String date, String time) {
        getInstance().show(Type.APPOINTMENT, "⏰ Rappel RDV", "Aujourd'hui · " + date);
    }
    ...
    public void show(Type type, String title, String body) {
        Platform.runLater(() -> {
            try {
                // ── Stage sans owner pour éviter le blocage ───────────────────
                ...
                // PAS de initOwner() — cause le non-affichage sur Windows
                ...
                System.out.println("🔔 Notif affichée → " + title);

            } catch (Exception e) {
                System.err.println("❌ NotificationService: " + e.getMessage());
                e.printStackTrace();
            }
        });
    }
